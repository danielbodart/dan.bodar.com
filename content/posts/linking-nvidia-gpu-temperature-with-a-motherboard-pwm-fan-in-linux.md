...
So I have a closed loop water cooler on my GPU and I replaced the stock single speed fan with two Noctua PWM fans (doing push/pull) connected to my motherboard. Even though the fans can be seen by <code>lm_sensors</code> and <code>fancontrol</code> the Nvidia GPU does not appear as it's  proprietary driver.

I knew <code>nvidia-settings</code> could be used to query the temperature (and the pump speed if you care) so I wrote a script to tie it all together:
